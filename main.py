        ...
        if opcao_inicial == "1":
            ...
            while service.cliente_autenticado:
                ...
                if opcao_cliente == "1":
                    ...
                    if not contas:
                        view.exibir_erro("Você não possui contas. Abra uma nova conta primeiro.")
                        view.pausar()
                        continue

                    view.exibir_tabela_contas(contas)
                    numero_conta = IntPrompt.ask("\nDigite o número da conta que deseja acessar")
                    sucesso_conta, msg_conta, conta = service.selecionar_conta(numero_conta)
                    ...
                    while service.conta_ativa:
                        ...
                        if op_conta == "1":
                            ...
                            # Transferência
                            num_destino = IntPrompt.ask("Número da conta de destino")
                            valor = FloatPrompt.ask("Valor da transferência (R$)")
                            sucesso_op, msg_op = service.realizar_transferencia(num_destino, valor)
                            if sucesso_op:
                                view.exibir_sucesso(msg_op)
                            else:
                                view.exibir_erro(msg_op)
                            ...
                            # Rendimento Poupança
                            sucesso_op, msg_op = service.aplicar_rendimento_poupanca()
                            if sucesso_op:
                                view.exibir_sucesso(msg_op)
                            else:
                                view.exibir_erro(msg_op)
                    ...
                    sucesso_abertura, msg_abertura, _ = service.abrir_conta(cliente.cpf, tipo_nome)
                    if sucesso_abertura:
            ...
            if sucesso_cad:
                ...
                abrir = Prompt.ask("Deseja abrir sua primeira conta agora?", choices=["s", "n"], default="s")
                if abrir == "s":
                    tipo = Prompt.ask("Tipo de conta: [1] Corrente | [2] Poupança", choices=["1", "2"], default="1")
